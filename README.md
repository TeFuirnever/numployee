...
| **L4 自主专家** | 多员工编排（Team），生产级治理 | sequential/parallel 派发正确；A/B 评测与成本意识在位 | [probes/suites/](probes/suites/) L4 子集 |
| **L5 组织成员** | 自我进化、知识沉淀、生态参与 | 回归集常态化运行；人格配方可复用可分发 | 题目待 SIP 补充（当前见 suites/l5.md 证据要求） |
...
| **M1 SPEC v1.0** | SPEC.md rc2→正式（评审意见关闭或转 SIP）、探针双评审一致率研究（作为 v1.0 附录发布）、L5 探针子集回填 | 待启动 |
