...
>
> 版本：v1.0（与 SPEC v1.0 同步）｜ 日期：2026 年 9 月 ｜ 编制：numployee 项目
...
四要素与第 2 章三个缺口的对应是显式的：持久性缺口由**持久身份**直接回应；治理缺口由**可治理红线**直接回应；复用缺口有两个侧面，分别由两个要素回应——配置（提示词、工具、知识模板）锁在个人对话框里，对应**可装配能力**；专家经验无法沉淀为组织资产，对应**可沉淀知识**。缺口与要素是「3 → 4」而非一一对应：可装配能力回答「能力如何被工程化复用」，可沉淀知识回答「经验如何被组织化继承」，二者共用复用缺口这一症状，但机制与失效形态不同【观点】。
...
**近期（标准演进）**：本规范的标准条款变更只经 SIP 流程发生（`sips/`）；已排期方向见 README 路线图 M2——英文版学术白皮书、开放问题攻关（记忆遗忘判据、人格健康度量化）。参考实现一侧的计划不构成标准路线：该产品 V2 方向（作者自述）【实践】为 Team 引入 selector（LLM 路由）派发、Agent 间共享上下文开关、per-target Bastion 策略、Team 模板经市场分发【观点】。
